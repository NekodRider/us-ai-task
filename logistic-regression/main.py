...
def cost_func_reg(theta, X, y, lamb):
    m = y.size
...
def gradient_descent_reg(X, y, theta, alpha, lamb, num_iters):
    m = y.size
    tmp = theta
    for i in range(num_iters):
...
def newton_descent(X, y, theta, lamb, num_iters):
    m = y.size
    tmp = theta
    for i in range(num_iters):
        delta = 1 / (X.T.dot(sigmoid(X.dot(theta)) - y)).dot(cost_func_reg(tmp, X, y, lamb)[0] * m)
        tmp = tmp - delta
        for k in delta:
            print(k < 0.01)
            if k < 0.02:
